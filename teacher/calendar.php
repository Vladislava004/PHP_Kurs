...
$stmt = $pdo->prepare("SELECT l.*, c.title as course_title, c.category_id, cat.name as category_name
                       FROM lessons l 
...
<div class="card">
    <div class="card-header">
        <h2 class="card-title">Календарь моих занятий</h2>
        <div style="display: flex; gap: 1rem; align-items: center;">
            <a href="?month=<?php echo $prevMonth; ?>&year=<?php echo $prevYear; ?>" class="btn btn-sm btn-secondary">← Предыдущий месяц</a>
            <span style="font-weight: 600; font-size: 1.1rem;"><?php echo $monthNames[$month]; ?> <?php echo $year; ?></span>
            <a href="?month=<?php echo $nextMonth; ?>&year=<?php echo $nextYear; ?>" class="btn btn-sm btn-secondary">Следующий месяц →</a>
            <a href="?month=<?php echo date('n'); ?>&year=<?php echo date('Y'); ?>" class="btn btn-sm btn-primary">Текущий месяц</a>
        </div>
    </div>
                                ...
                                if (($week == 0 && $dow < $dayOfWeek) || $day > $daysInMonth):
                                    ...
                                    if ($hasLessons):
                                        foreach ($lessonsByDate[$currentDate] as $lesson):
                                            ...
                                            echo '<a href="view_lesson.php?id=' . $lesson['id'] . '" style="color: #333; text-decoration: none;">';
                                            echo htmlspecialchars(mb_substr($lesson['title'], 0, 30)) . (mb_strlen($lesson['title']) > 30 ? '...' : '');
                                            echo '</a>';
                                            echo '</div>';
                                        endforeach;
                                    endif;
